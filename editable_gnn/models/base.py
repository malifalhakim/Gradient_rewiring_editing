...
class BaseModel(torch.nn.Module):
    ...
    def from_pretrained(cls, in_channels: int, out_channels: int, saved_ckpt_path: str, **kwargs):
        ...
        if not saved_ckpt_path.endswith('.pt') and not saved_ckpt_path.endswith('.pth'):
    ...
    @classmethod
    def load_pretrained(cls, in_channels: int, out_channels: int, saved_ckpt_path: str, **kwargs):
        model = cls(in_channels=in_channels, out_channels=out_channels, **kwargs)
        print(f'load model weights from {saved_ckpt_path}')
        model.load_state_dict(torch.load(saved_ckpt_path, map_location='cpu'))
        return model
    ...
    def mini_inference(self, x_all, loader):
        ...
        for i in range(len(self.convs)):
            ...
            for batch_size, n_id, adj in loader:
                gree_index, _, size = adj.to('cuda')
                x = x_all[n_id].to('cuda')
                xs.append(self.forward_layer(i, x, gree_index, size).cpu())
